...
// SingleBillBook.jsx - Move to Design Button Always Enabled (No Validation)
import React, { useState, useEffect, useCallback } from "react";
import { useParams, useNavigate } from "react-router-dom";
import Navbar from "./Navbar";
import Footer from "./Footer";
...
const SingleBillBook = () => {
  ...
  const handleMoveToDesign = () => {
    ...
    const designData = {
      billBookId: id,
      companyName: customerDetails.companyName || billBook?.companyName || "My Company",
      address: customerDetails.address || "",
      mobile: customerDetails.mobile || "",
      email: customerDetails.email || "",
      gstNo: customerDetails.gstNo || "",
      logo: customerDetails.logo,
      design: billBook?.design || {},
      templateImage: billBook?.templateImage || null,
      selectedOptions: {
        billBook: selectedBillBook,
        billBookType: selectedBillBookType,
        bookContains: selectedBookContains,
    ...
    localStorage.setItem('billBookDesignData', JSON.stringify(designData));
    
    // Navigate to Canvas Editor
    navigate('/add-design', { state: { fromBillBook: true, billBookData: designData } });
  ...
  const handleAddToCart = () => {
    ...
    const cartItem = {
      ...
      userId: userId,
      productId: billBook?._id,
      name: customerDetails.companyName || "Bill Book",
      image: getImageUrl(),
      quantity: quantity,
      price: calculatePrice(),
      customerDetails: {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#e0eafc] to-[#cfdef3]">
        ...
        <div className="flex justify-center items-center h-screen">
          <div className="text-center backdrop-blur-sm bg-white/40 p-8 rounded-3xl shadow-[12px_12px_24px_#b8b9be,_-12px_-12px_24px_#ffffff] border border-white/50 max-w-md">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">Error Loading Bill Book</h3>
            <p className="text-gray-700 mb-4">{error || "Bill book not found"}</p>
            <button onClick={() => navigate("/billbooks")} className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-2 rounded-full hover:shadow-xl transition-all duration-300">Back to Bill Books</button>
          </div>
        </div>
        <Footer />
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#e0eafc] to-[#cfdef3]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <div className="backdrop-blur-sm bg-white/40 rounded-3xl p-6 shadow-[12px_12px_24px_#b8b9be,_-12px_-12px_24px_#ffffff] border border-white/50">
                ...
                <div className="relative bg-gradient-to-br from-white/60 to-gray-100/60 rounded-2xl p-4 mb-4 min-h-[400px] flex items-center justify-center shadow-inner">
                  <img src={getImageUrl()} alt={billBook.companyName} className="max-h-[350px] max-w-full object-contain transform hover:scale-105 transition-transform duration-500" onError={handleImageError} />
                  {billBook.status === 'active' && (
                    <div className="absolute top-4 left-4 bg-gradient-to-r from-green-500 to-emerald-500 text-white text-xs font-bold px-3 py-1 rounded-full flex items-center shadow-lg">
                      <FaCheckCircle className="mr-1" /> ACTIVE
                    </div>
              ...
              <div className="backdrop-blur-sm bg-white/40 rounded-3xl p-6 shadow-[12px_12px_24px_#b8b9be,_-12px_-12px_24px_#ffffff] border border-white/50">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {billBook.design?.roundedCorners && <span className="text-xs px-2 py-1 bg-green-500/20 text-green-700 rounded-full">Rounded Corners</span>}
                  {billBook.design?.shadow && <span className="text-xs px-2 py-1 bg-blue-500/20 text-blue-700 rounded-full">Shadow Effect</span>}
                  {billBook.design?.border && <span className="text-xs px-2 py-1 bg-purple-500/20 text-purple-700 rounded-full">Border</span>}
                  {billBook.logoSettings?.show && <span className="text-xs px-2 py-1 bg-orange-500/20 text-orange-700 rounded-full">Logo Included</span>}
                </div>
              </div>
            </div>
              ...
              <div className="backdrop-blur-sm bg-white/40 rounded-3xl p-6 shadow-[12px_12px_24px_#b8b9be,_-12px_-12px_24px_#ffffff] border border-white/50">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={idx}>
                      <label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center">{field.icon}{field.label}</label>
                      <select value={field.value} onChange={(e) => field.setter(e.target.value)} className="w-full p-3 bg-white/50 backdrop-blur-sm border border-white/30 rounded-xl focus:ring-2 focus:ring-indigo-500 shadow-inner cursor-pointer">
                        <option value="">Select {field.label}</option>
      ...
      {showPreviewModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-fadeIn">
          <div className="relative max-w-5xl w-full max-h-[90vh] bg-gradient-to-br from-white to-gray-100 rounded-3xl shadow-2xl overflow-hidden animate-scaleIn">
            <div className="flex justify-between items-center p-6 border-b border-gray-200 bg-gradient-to-r from-indigo-600 to-purple-600">
              <h3 className="text-xl font-bold text-white flex items-center gap-2"><FaEye className="text-white" />Bill Book Preview</h3>
              <div className="flex gap-2">
            ...
            <div className="p-6 overflow-auto max-h-[calc(90vh-80px)]">
              {previewError ? (<div className="text-center py-12"><div className="text-red-500 text-4xl mb-4">⚠️</div><p className="text-gray-700">{previewError}</p></div>) : (
                <div className="flex justify-center items-center min-h-[400px]"><img src={previewImage} alt="Bill Book Preview" className="max-w-full max-h-[70vh] object-contain rounded-lg shadow-lg" onError={() => setPreviewError("Failed to load preview image")} /></div>
              )}
            </div>
            <div className="p-4 border-t border-gray-200 bg-gray-50/50"><p className="text-center text-sm text-gray-600">This is how your bill book will look after printing</p></div>
          </div>
        </div>
      )}
